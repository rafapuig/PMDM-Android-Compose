...
import es.rafapuig.pmdm.compose.viewmodel.with_dependecies.dice.ui.model.toUi
...
class DiceViewModel(
    ...
    val _uiState = savedStateHandle.getMutableStateFlow("uiState", DiceUiState.Companion.Empty)
    val uiState = _uiState.asStateFlow()

    private var rollingDiceJob: Job? = null

    init {
        if (uiState.value == DiceUiState.Companion.Empty) roll()
    }
    ...
    object FactoryNotGood : ViewModelProvider.Factory {
        override fun <T : ViewModel> create(
            ...
            return DiceViewModel(
                repository = DiceRepositoryImpl(), // Implementacion harcodeada
                savedStateHandle = savedStateHandle
            ) as T
        }
    }
